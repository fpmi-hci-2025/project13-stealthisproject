...
```puml
@startuml
' EventStorming.puml based on standard conventions
...
title Event Storming: Process Modeling

Command(book, "Забронировать место")
Aggregate(booking, "Бронирование")
DomainEvent(booked, "Место забронировано")
Policy(payment_policy, "Политика оплаты (15 мин)")
Command(pay, "Оплатить")

book -> booking
booking -> booked
booked -> payment_policy
payment_policy -> pay
@enduml
```
