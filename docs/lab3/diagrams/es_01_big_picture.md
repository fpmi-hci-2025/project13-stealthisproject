...
Actor(user, "Пассажир")
Command(search, "Поиск рейса")
DomainEvent(searched, "Рейсы найдены")

Command(select, "Выбор места")
DomainEvent(selected, "Место выбрано")

Command(pay, "Оплата")
DomainEvent(paid, "Билет оплачен")

DomainEvent(ticket_ready, "Билет готов")
